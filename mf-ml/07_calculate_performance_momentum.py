...
DEFAULT_TARGET_TABLE = "performance_momentum_features_v2"
FACTOR_SCHEMA = "mf100"
...
def read_daily_navs(engine, schema: str, source_table: str) -> pd.DataFrame:
    ...
    frame = pd.read_sql(
        text(
            f'''
            SELECT "schemeName" AS scheme_name, report_date, "navDirect" AS nav_direct
            FROM {source}
...
def _fit_ols_window(
    ...
    r_squared = max(0.0, 1.0 - (rss / tss)) if tss > 1e-12 else np.nan
