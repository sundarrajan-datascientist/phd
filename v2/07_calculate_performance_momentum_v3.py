...
SOURCE_TABLE = f"crisil_fund_daily_cleaned_{RUN_TIMESTAMP}"
DEFAULT_TARGET_TABLE = f"performance_momentum_features_v3_{RUN_TIMESTAMP}"
    ...
    frame = pd.read_sql(
        text(
            ...
            SELECT
                scheme_name,
                report_date,
                nav_direct_cleaned AS nav_direct
            FROM {source}
            WHERE scheme_name IS NOT NULL
              AND report_date IS NOT NULL
              AND nav_direct_cleaned IS NOT NULL
            '''
        ),
        engine,
    )
