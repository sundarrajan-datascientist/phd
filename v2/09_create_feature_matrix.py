...
PERFORMANCE_TABLE = (
    f"performance_momentum_features_v3_{RUN_TIMESTAMP}"
)
...
TARGET_TABLE = f"feature_matrix_2019_2025_{RUN_TIMESTAMP}"
...
def create_feature_matrix(engine) -> pd.DataFrame:
    ...
    WITH monthly_performance AS (
        ...
        FROM (
            ...
            FROM {SCHEMA}.{PERFORMANCE_TABLE} p

            WHERE p.trade_date >= DATE '2019-01-01'
              AND p.trade_date <  DATE '2026-01-01'
        ) x
    ...
    INNER JOIN {SCHEMA}.{AUM_FLOW_TABLE} a
